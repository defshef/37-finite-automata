...
sampleRules = [(1, 'a', 2), (1, 'b', 1),
              (2, 'a', 2), (2, 'b', 3),
              (3, 'a', 3), (3, 'b', 3)]
sampleDfa = (1, [1, 3], sampleRules)

modifiedDfa = (1, [3], sampleRules)
...
main = hspec $ do
  ...
  describe "acceptChar" $ do
    it "returns a new Dfa when new char matches" $ acceptChar sampleDfa 'a' `shouldBe` Just (2, [1, 3], sampleRules)
